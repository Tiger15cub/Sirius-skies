...
export default async function Athena(
  ...
  try {
    ...
    const AthenaData = await AddProfileItem(
      athena,
      profileId,
      rvn,
      level,
    ...
    if (isUpdated) {
      log.debug("Updated Profile for new Cosmetics.", "ProfileAthena");
      return JSON.parse(
    ...
    if (!fs.existsSync(path.join(__dirname, "profile.json")))
      ...
      existingData.profileChanges[0].profile.items = {
        ...existingData.profileChanges[0].profile.items,
        ...newData.profileChanges[0].profile.items,
      };
