...
export default async function Athena(
  ...
  try {
    ...
    const items: Array<Array<Record<string, any>>> = athena.items;
    ...
    const storedUser = storedItems.find(
      (user: any) => user.accountId === accountId
    );

    if (!storedUser) {
      storedItems.push({ accountId, items: [] });
    }

    // for (const innerItems of items) {
    //   for (const item of innerItems) {
    //     let templateKey: string | null = null;

    //     for (const key of Object.keys(item)) {
    //       const index = key.indexOf("Athena");
    //       if (index !== -1) {
    //         templateKey = key.substring(index);
    //         break;
    //       }
    //     }

    //     if (templateKey) {
    //       const templateItem = item[templateKey] as Record<string, any>;
    //       const templateId = templateItem["templateId"];
    //       const itemAttributes = templateItem["attributes"] as Record<
    //         string,
    //         any
    //       >;

    //       if (templateId !== null && itemAttributes !== null) {
    //         const existingItem =
    //           AthenaData.profileChanges[0].profile.items[templateKey];
    //         if (
    //           !existingItem ||
    //           !isEqual(existingItem.attributes, itemAttributes)
    //         ) {
    //           AthenaData.profileChanges[0].profile.items[templateKey] = {
    ...
    //         const storedItem = storedUser?.items.find((item: any) =>
    //           item.hasOwnProperty(templateKey as string)
    //         );

    //         if (
    //           !storedItem ||
    //           !isEqual(storedItem[templateKey].attributes, itemAttributes)
    //         ) {
    ...
    fs.writeFileSync(`${__dirname}/items.json`, JSON.stringify(storedItems));
